...
This project includes a Bash-based test script to validate the Terraform configuration by deploying infrastructure to GKE and verifying it behaves as expected.
...
1. *Configures Terraform execution*
   ...
   - Optionally checks and applies formatting (=terraform fmt=)
...
3. *Registers automatic cleanup*
   - Ensures =terraform destroy= is executed at the end of the script
   - Cleanup runs even if the test fails (via =trap=)

4. *Fetches Terraform outputs*
   - Reads the health-check URL output from Terraform
   - This URL represents the exposed application endpoint (supports HTTP or HTTPS)

APPLIES HELM GITOPS APPLICATIONSET
