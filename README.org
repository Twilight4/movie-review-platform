...
*Secrets*:
- =TERRAFORM_SA_KEY= – Service Account key used by Terraform
...
**** Testing with [[https://github.com/gruntwork-io/terratest][Terratest]]
This project uses *Terratest* to validate the Terraform configuration by deploying infrastructure to GKE and verifying it behaves as expected.

The test runs against the example configuration located in: =terraform/tests/terratest=.
Go module dependencies are locked in =go.sum= to ensure consistent and reproducible test runs across environments.

***** Prerequisites
Before running the tests, ensure you have:
- Go installed (version compatible with Terratest)
- Terraform installed
- Valid GCP credentials available in your environment
- Permissions to create and destroy GKE-related resources

***** Running the Tests
#+begin_src shell
# Navigate to the terratest directory
cd terraform/tests/terratest

# View task commands
go-task --list-all

# Download GO dependencies and run test
go-task 01-download-dependencies
go-task 02-run-test
#+end_src

***** What the Terratest Script Does
The =test.go= script performs the following steps:
1. *Configures Terraform execution*
   - Points Terratest to the Terraform example located at:
