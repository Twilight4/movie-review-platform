...
│   │   └── Taskfile.yaml
│   └── Taskfile.yaml
├── cicd                   → GitOps configuration (ArgoCD)
│   ├── github-actions
│   │   └── Taskfile.yaml
│   ├── argocd-gitops
│   │   └── apps
...
Rendered from =assets/render_architecture.py= — run =uv run assets/render_architecture.py= to regenerate =assets/architecture.png=.

#+CAPTION: Full architecture — git push triggers CI; CI builds and pushes the image; CD provisions cloud infrastructure (Terraform) and syncs manifests (ArgoCD) into GKE.
...
*** CI/CD
- Build, test, check formatting and find bugs in application code in CI pipeline
...
- =PROJECT_ID= – GCP project name you deployed
...
- Go to GKE > Clusters > Your cluster
- Check "Labels" section
- Should see: =environment=, =managed_by=, =project=, =team=
...
*** Verify Observability setup (Grafana / Prometheus / Alloy)
#+begin_src shell
# Check if pods are running for kube-prometheus-stack and loki
kubectl get pods -n monitoring

# Check if pods are running for alloy agent
...
# Verify Alloy ConfigMap content
kubectl get configmap alloy -n observability -o yaml

# Verify it’s sending data by checking alloy logs (Look for: remote_write succeeded ; loki write succeeded)
kubectl logs -n observability daemonset/alloy

# Loki readiness
kubectl get pods -n monitoring -l app.kubernetes.io/name=loki

# Check prometheus targets & access Grafana WebUI
cd k8s/helm/monitoring
go-task port-forward-prometheus      # confirm targets are up at http://localhost:9090/targets, for alerts: http://localhost:9090/alerts
go-task port-forward-grafana
go-task port-forward-alloy           # validate enabled components

# Access ArgoCD UI and get default password
go-task port-forward-argocd
go-task get-argocd-password

### Test Monitoring Alerts
# Test pod restart alert (optional)
kubectl delete pod -n staging -l app=movie-api

# Watch for alerts in Prometheus - Open: http://localhost:9090/alerts
# Check AlertManager - Open: http://localhost:9093
...
Grafana, Prometheus:
- [X] Grafana deployment with Alloy agent
- [X] Prometheus data source
- [X] Alertmanager deployment
- [X] Node Exporter metrics for system performance monitoring
- [X] Docker monitoring metrics
- [X] System logs collection with Loki
